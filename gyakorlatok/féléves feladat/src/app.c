...
void init_app(App* app, int width, int height)
{
    ...
    app->is_running = true;
    app->scene.is_menu_open = false; //The menu is not shown when starting the program
}
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDL_SCANCODE_E:
                ...
                break;
            case SDL_SCANCODE_M:
                if(!(app->scene.is_menu_open)){
                    app->scene.is_menu_open = true;
                }else{
                    app->scene.is_menu_open = false;
                }
                break;
            case SDL_SCANCODE_L:
                if(app->scene.light_sensitivity == 1.0){
                    app->scene.light_sensitivity = 0.0;
                }else{
                    app->scene.light_sensitivity = 1.0;
                }
                break;
...
void render_app(App* app)
{
    ...
    glPushMatrix();
    set_view(&(app->camera));
    render_scene(&(app->scene), app->window);
    glPopMatrix();
