...
void update_pong(Pong* pong, double time)
{
    ...
    update_ball(&(pong->ball), time, pong->ball.radius, pong->ball.rotation);
    bounce_ball(pong);
}
...
void bounce_ball(Pong* pong)
{
    if (pong->ball.x - pong->ball.radius < 50) {
        if(pong->ball.y >= pong->left_pad.y-50 && pong->ball.y <= (pong->left_pad.y + pong->left_pad.height)+50){
            pong->ball.x = pong->ball.radius + 50;
            pong->ball.speed_x *= -1;
            //pong->ball.rotation -= 10;
        }else{
            pong->ball.x = 600;
            pong->ball.y = 300;
            pong->right_pad.points += 1;
        }
    }
    if (pong->ball.x + pong->ball.radius > pong->width - 50) {
        if(pong->ball.y >= pong->right_pad.y-50 && pong->ball.y <= (pong->right_pad.y + pong->right_pad.height)+50){
